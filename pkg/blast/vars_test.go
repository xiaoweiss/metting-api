...
func TestRecipientVars_DashboardImage(t *testing.T) {
	...
	vars, inlineImages, skipReason, hotelId := recipientVars(db, c, "1369030562@qq.com", now, 0)

	fmt.Printf("vars[Date]=%v\nvars[HotelName]=%v\nvars[OccupancyRate]=%v\nvars[DashboardImage]=%v\nhotelId=%v\nskipReason=%q\ninlineImages=%+v\n",
		vars["Date"], vars["HotelName"], vars["OccupancyRate"], vars["DashboardImage"],
		hotelId, skipReason, inlineImages)

	if skipReason != "" {
		t.Errorf("应该不 skip, 但 skipReason=%q", skipReason)
	}
	if hotelId != 1 {
...
func TestRecipientVars_SnapshotMissing(t *testing.T) {
	...
	_, inlineImages, skipReason, hotelId := recipientVars(db, c, "1369030562@qq.com", missingDay, 0)

	if skipReason != SkipReasonSnapshotMissing {
		t.Errorf("缺图应该 skip, skipReason=%q", skipReason)
	}
	if hotelId != 1 {
		t.Errorf("hotelId 应=1, 实际=%d", hotelId)
	}
	if len(inlineImages) != 0 {
		t.Errorf("inlineImages 应空, 实际=%d", len(inlineImages))
	}
}
